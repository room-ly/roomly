    ...
    <>
      <div className="card card-interactive p-4 relative">
        <div className="absolute top-3 right-3" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 bg-card border border-border-light rounded-lg shadow-lg py-1 z-10 min-w-[120px]">
              <button
                onClick={() => {
                  setMenuOpen(false);
                  setEditOpen(true);
                }}
                className="w-full flex items-center gap-2 px-3 py-2 text-[13px] hover:bg-bg-secondary transition-colors"
              >
                <Pencil size={13} /> 編集
              </button>
              <button
                ...
                disabled={deleting}
                className="w-full flex items-center gap-2 px-3 py-2 text-[13px] text-danger hover:bg-bg-secondary transition-colors"
              >
                <Trash2 size={13} /> 削除
              </button>
            </div>
        ...
        <div className="flex gap-2 text-center mb-4">
          ...
          <div className="flex-1 py-1.5 rounded bg-bg-secondary">
            ...
            <p className="text-[15px] font-semibold text-success tabular-nums">{owner.occupiedCount}</p>
          </div>
        </div>
        ...
        <div className="border-t border-border-light pt-3 space-y-1.5 text-[13px]">
          ...
          {owner.expenseDeducted > 0 && (
            <div className="flex justify-between">
              ...
              <span className="text-warning font-medium tabular-nums">-¥{owner.expenseDeducted.toLocaleString()}</span>
            </div>
          )}
