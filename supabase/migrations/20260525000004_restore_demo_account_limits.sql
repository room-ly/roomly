-- デモアカウント用の制限管理（復活）
--
-- 経緯: 旧 _archive/00040_demo_account_limits.sql で追加していた
-- companies.is_demo / demo_write_logs / reset_demo_data() が、
-- baseline 集約（20260525000000_baseline.sql）への転記時に取りこぼされ、
-- 本番DBに存在しない状態だった。
-- このため checkDemoLimit()（kanri/src/lib/supabase-server.ts）が
-- companies.is_demo を参照して常にエラーになっていた。
-- database.types.ts には型が残っているため、本ファイルでスキーマ側を復元する。
-- すべて IF NOT EXISTS / OR REPLACE で冪等。

-- companies に is_demo フラグ追加
alter table public.companies
  add column if not exists is_demo boolean not null default false;

-- デモ会社の書き込み回数を追跡するテーブル
...
drop policy if exists demo_write_logs_tenant_policy on public.demo_write_logs;
