-- rent_billings.status から「overdue」を廃止する。
--
-- 背景:
--   「滞納（overdue）」は due_date と支払い実額から派生できる状態で、DBに別ステータスとして
--   持つと「未入金（unpaid）」との二重管理になり、cron が止まれば古いまま放置される。
--   Single Source of Truth として lib/billing-status.ts の deriveBillingStatus() で派生させる
--   方針に統一する。
--
--   このマイグレーションでは:
--     1. 既存の status='overdue' 行を 'unpaid' に書き換え
--     2. reset_demo_data() 関数からも 'overdue' を 'unpaid' に置換
...
-- 2. デモシード関数も unpaid に統一（既存定義の overdue 分岐を unpaid に）
...
declare
  ...
  -- 月ごとの未収件数。overdue は派生扱いになったので overdue/unpaid を区別せず unpaid のみ。
...
begin
  ...
  for v_idx in 1..6 loop
    ...
    for i in 1..9 loop
      ...
      -- ステータスは unpaid / paid のみ。「滞納」は due_date 超過から派生させる。
