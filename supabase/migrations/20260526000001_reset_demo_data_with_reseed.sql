-- reset_demo_data() に「削除後の再シード」を組み込む。
--
-- 背景:
--   既存の reset_demo_data() は削除のみで再生成しない設計だった。
--   そのため Edge Function (cron) が走るとデモ会社が空になり、
--   ダッシュボードの「今月の回収 ¥0 / 回収率 0%」など、見栄えが崩れる。
--
-- このマイグレーションでの変更:
--   1. 既存の削除ロジックは保持
--   2. 削除後に「mock-data.ts 同等の固定データ」をINSERTし直す
--   3. 家賃請求（rent_billings）と入金（rent_payments）は実行時の current_date を基準に
--      直近6ヶ月を動的生成（金額の小さい1〜2件を未収に振り、当月は約86%回収）
--   4. 入金のnotesには「振込元: <銀行> <支店> 普通 <名義>」を入れる
--   5. すべて IF NOT EXISTS / OR REPLACE で冪等
--
-- これにより、毎週月曜0:00のcronでリセットされても、
-- ダッシュボードが「使われている画面」として埋まった状態を再現できる。
